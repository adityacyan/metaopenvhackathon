...
class OpenAPIValidator:
    ...
    def validate(schema_dict: Dict[str, Any]) -> List[ValidationError]:
        ...
        if "openapi" in schema_dict:
            ...
            if not isinstance(openapi_version, str) or not re.match(r"^3\.[0-1]\.\d+$", openapi_version):
                errors.append(ValidationError(
        ...
        if "paths" in schema_dict:
            ...
            if not isinstance(paths, dict):
                ...
                for path_name, path_item in paths.items():
                    ...
                    operations = {k: v for k, v in path_item.items() 
                    ...
                    for method in path_item.keys():
                        if method not in OpenAPIValidator.VALID_HTTP_METHODS and not method.startswith("x-"):
                            errors.append(ValidationError(
    ...
    def validate(schema_dict: Dict[str, Any]) -> List[ValidationError]:
        ...
        for path_name, path_item in paths.items():
            ...
            for method, operation in path_item.items():
                ...
                if method == "get" and any(word in operation_id + summary + description for word in ["create", "add", "new", "post"]):
                    errors.append(ValidationError(
                ...
                if method in ["post", "put", "patch"] and any(word in operation_id + summary + description for word in ["list", "get", "retrieve", "fetch"]):
                    errors.append(ValidationError(
                        error_type="incorrect_http_method",
                        severity="warning",
                        path=f"paths.{path_name}.{method}",
...
class ValidationPipeline:
    ...
    def validate(schema_json: str) -> ValidationResult:
        ...
        if isinstance(parse_result, ValidationError):
            ...
            validation_stages["json_parser"] = {"passed": False, "errors": [parse_result]}
        ...
        validity_score = 0.0 if critical_errors else 1.0
        
        # Best practices score based on info and warning errors
        info_warning_errors = [e for e in all_errors if e.severity in ["warning", "info"]]
        max_bp_errors = 20
        best_practices_score = max(0.0, 1.0 - (len(info_warning_errors) / max_bp_errors))
