...
try:
    ...
    try:
        ...
        from api_conformance_gym.models import APIAction, APIObservation, APIState, ValidationResult
...
class APIEnvironment(Environment):
    ...
    def reset(self, seed: Optional[int] = None) -> APIState:
        """
        ...
        Returns:
            APIState with new business requirement and reset counters
        """
    ...
    def step(self, action: APIAction) -> Tuple[APIObservation, float, bool, Dict[str, Any]]:
        """
        ...
        episode_done = (
            self._current_state.iteration_count >= self.MAX_ITERATIONS or
            (validation_result.validity_score == 1.0 and validation_result.best_practices_score == 1.0)
        )
        ...
        if episode_done:
            episode_info.update({
                "termination_reason": "max_iterations" if self._current_state.iteration_count >= self.MAX_ITERATIONS else "perfect_schema",
        ...
        return observation, reward, episode_done, episode_info
    ...
    def _generate_feedback(self, validation_result: ValidationResult) -> str:
        ...
        warning_errors = [e for e in validation_result.errors if e.severity == "warning"]
        info_errors = [e for e in validation_result.errors if e.severity == "info"]
        
        feedback_parts = []
        
        if critical_errors:
            feedback_parts.append(f"Critical issues ({len(critical_errors)}): {critical_errors[0].message}")
            if len(critical_errors) > 1:
                feedback_parts.append(f"Plus {len(critical_errors) - 1} more critical issues.")
        
        if warning_errors:
            feedback_parts.append(f"Warnings ({len(warning_errors)}): {warning_errors[0].message}")
        
        if info_errors:
            feedback_parts.append(f"Suggestions ({len(info_errors)}): {info_errors[0].message}")
